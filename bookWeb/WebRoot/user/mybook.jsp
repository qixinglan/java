...
<script type="text/javascript" src="http://127.0.0.1:8081/bookWeb/js/jquery.js"></script>
...
<script type="text/javascript">                           
function jump(){
 var a=document.getElementById("jumppage");
 var b=document.getElementById("hiddenuid");
 if(a.value!=""){
 window.location.href="http://127.0.0.1:8081/bookWeb/mybook.do?currpage="+a.value+"&u_id="+b.innerText;
 }
}
</script>
</head>
...
//type=new String(type.getBytes("ISO-8859-1"),"utf-8");//当前类型
...
<dl class="bookitem">
	<dt>
		...
		<img alt="增寿延命之法" style="width:90px;height:128px;" src="${e.imgadd}" />
		
	</dt>
	<dd>
		<ul>
			<li class="stitle"><h1><a target="_blank" href="http://127.0.0.1:8081/bookWeb/read.do?bid=${e.bid}&u_id=<%=user.getUid()%>">${e.bname}</a> </h1>
			</li>
			...
			<li class="detail"><a target="_blank" href="http://127.0.0.1:8081/bookWeb/read.do?bid=${e.bid}&u_id=<%=user.getUid()%>">${e.introduce}</a>
			</li>
		</ul>
	</dd>
</dl>
...
<table align="center">
<tr>
<td style="display:none" id="hiddenuid">${user.uid}</td>
<td>当前第<%=currpage%>页</td>
<td><a href="http://127.0.0.1:8081/bookWeb/mybook.do?currpage=<%=currpage-1%>&u_id=${user.uid}">上一页</a></td><td><a href="http://127.0.0.1:8081/bookWeb/mybook.do?currpage=<%=currpage+1%>&u_id=${user.uid}">下一页</a></td>
<td>总共</td><td><%=allpage %></td><td>页</td><td><a href="javascript:;" onclick="jump()">跳转</a></td><td><input id="jumppage" size="2"/></td><td>页</td>
</tr>
</table>
</div>
