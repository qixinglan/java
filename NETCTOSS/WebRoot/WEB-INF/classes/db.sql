...
1,'5.9元套餐',20,5.9,0.4,0,'5.9元20小时/月，超出部分0.4元、小时'
,sysdate ,null,null);
select * from cost;
alter table cost drop (costtype);
alter table cost add(costtype char default '1');
update account set status='0' where account_id=1022;
...
update service set pause_date=sysdate where service_id=2001;
...
commit;
select a.admin_id,a.name,r.name,m.name from admin_info a left join 
admin_role ar on a.admin_id=ar.admin_id left join
role_info r on ar.role_id=r.role_id left join 
role_module rm on r.role_id=rm.role_id left join 
module_info m on rm.module_id=m.module_id;
...
where a.admin_id=2000 )order by module_id;
