...
@Service
public class FxryNoticeReportServiceZFDC extends IReportServiceNew {
	@Autowired
	...
	private String getReportUrl() {
		...
		criterions.add(Restrictions.eq("reportType","WJS"));
		List<SupervisionModule> list = viewReportFxryInfoService.findByCriteria(criterions,
				SupervisionModule.class, null, null);
	...
	public ReportData getReportData(Bmb org) {
		List<Criterion> criterions = new ArrayList<Criterion>();
		criterions.add(Restrictions.lt("reportTime", new Date()));
		criterions.add(Restrictions.eq("state", FXRYState.ZJ));
		List<Order> orders = new ArrayList<Order>();
		ProjectionList projectionList = Projections.projectionList();
		projectionList.add(Projections.rowCount());
		Map<String, String> orgMap = new HashMap<String, String>();
		String orgId = org.getBm();
		String detail = "人员未报到统计数：";
		String parameter = "";
		if (org.isQxsfj()) {
			parameter = "sfsId";
		...
		List list = viewReportFxryInfoService.findByCriteria(criterions,
				ViewFxryNotice.class, projectionList, orders);
