...
import com.nci.sfj.xmpp.codec.XMLWriter;
		...
		synchronized (this) {
			if (!isClosed()) {
				...
				if (session != null) {
					...
					//--------不再全移除，在下面的监听器中移除-------
					// ((ClientSession) session).clearAllUser();
				}
				XmppClientManage.getInstance().closeSession(ioSession.getId());
	...
	public void deliver(Packet packet) {
		...
		if (!isClosed()) {
			...
			try {
				...
				buffer.get(context);
				String reply = new String(context, Charset.forName("UTF-8"));
				reply = reply.replaceAll(" xmlns=\"\"", "");
