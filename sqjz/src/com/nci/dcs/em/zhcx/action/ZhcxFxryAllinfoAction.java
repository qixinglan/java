...
import com.nci.dcs.em.zhcx.service.ZhcxFxryAllinfoService;
 ...
 * @since 
 */
public class ZhcxFxryAllinfoAction  extends BaseAction<ViewCcFxryAllinfo>{
	...
	@Autowired
	private ZhcxFxryAllinfoService service;
	 ...
	 * @return
	 */
	public ZhcxFxryAllinfoService getService() {
		return this.service;
	}
	 ...
	 * @return
	 */
	public void setService(ZhcxFxryAllinfoService service) {
...
//			}
	 ...
	 * @message:
	 */
	public String searchTongjiByFilters() throws Throwable {
		Bmb org = getCurOrg();
		String orgId = org.getBm();
		String filters = request.getParameter("filters");
		
		JQGridSearchRuleParser parser = new JQGridSearchRuleParser(filters);
		List<Criterion> criterions = parser.parse(ViewCcFxryAllinfo.class);
		...
		if(org.isSj()){
			...
			List<Criterion> criterions2 = parser.parse(ViewCcFxryAllinfo.class);
			...
			if(orgInfo.getOrgType().equals("2")||orgInfo.getOrgType().equals("3")){
				...
				Criteria criteria= service.createCriteria(criterions,
						ViewCcFxryAllinfo.class, projectionList,null);
	...
	public String searchTongji() throws Throwable {
		Bmb org = getCurOrg();
		String orgId = org.getBm();
		this.getRequestPage();
		String filters = this.getJqgrid().getFilters();
		...
		Criteria criteria2= service.createCriteria(criterions2,
				ViewCcFxryAllinfo.class, projectionList2,null);
		...
		List<ViewCcFxryAllinfo> listAll=criteria.list();
		
		List<ViewCcFxryAllinfo> fxryList = new ArrayList<ViewCcFxryAllinfo>();
		for (Object obj : listAll) {
			Object[] object = (Object[]) obj;
			ViewCcFxryAllinfo temp = new ViewCcFxryAllinfo();
		...
		ViewCcFxryAllinfo tempqb = new ViewCcFxryAllinfo();
		int qb = 0;
	...
	//导出
	private static final String[] headers = new String[] { "机构名称",
		"统计数据"};
public String exportTongJiExcel(){
String rowData;
try {
	rowData=new String(request.getParameter("rowData").getBytes("iso-8859-1"),"utf-8");
	...
	for (int i = 0; i <data.length; i+=2) {
		...
		content.add((i/2)+1);//序号
	    content.add(data[i]);
	    content.add(data[i+1]);
		...
		try {
			...
			for(int i=1;i<=bigPages;i++){
				...
				for(int k=0;k<res.size();k++){
					if(head.length!=1){
						ViewCcFxryAllinfo=(Object[])res.get(k);
					}else{//若是只选择一列导出，返回不再是数组而是string
						ViewCcFxryAllinfo=new Object[]{res.get(k)};
					}
					for(int t=0;t<head.length;t++){
						if(ViewCcFxryAllinfo[t]==null){
							ViewCcFxryAllinfo[t]="";
						}else{//数据格式化
							if(head[t].equals("responseOrg")){//单位格式化
								colo=(Object)getOrgName((String)ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("sex")){//性别格式化
								colo=dictionaryInfoService.findItemsMap("XB").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("nation")){//民族格式化
								colo=dictionaryInfoService.findItemsMap("MZ").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("state")){//矫正状态格式化
								colo=dictionaryInfoService.findItemsMap("JZRYZT").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("birthdate")
								...
								SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
								ViewCcFxryAllinfo[t]=((Date)ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=sdf.format(ViewCcFxryAllinfo[t]);
								continue;
							}
							if(head[t].equals("isadult")//是否成年格式化
								...
								colo=dictionaryInfoService.findItemsMap("SF").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							/*if(head[t].equals("isdeviceCode")){//是否电子监管格式化
								ViewCcFxryAllinfo[t]=dictionaryInfoService.findItemsMap("SF").get(ViewCcFxryAllinfo[t]);
							}*/
							if(head[t].equals("health")){//健康状况格式化
								colo=dictionaryInfoService.findItemsMap("JKZK").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("healthContagion")){//具体传染病史格式化
								colo=dictionaryInfoService.findItemsMap("CRB").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("politicsStatus")||head[t].equals("politicsStatusOriginal")){//（原）政治面貌格式化
								colo=dictionaryInfoService.findItemsMap("ZZMM").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("natureHome")){//户籍性质格式化
								colo=dictionaryInfoService.findItemsMap("HJXZ").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("educationDegree")){//文化程度格式化
								colo=dictionaryInfoService.findItemsMap("WHCD").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("maritalState")){//婚姻状况格式化
								colo=dictionaryInfoService.findItemsMap("HYZK").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("workState")){//就业就学情况格式化
								colo=dictionaryInfoService.findItemsMap("JYJX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("adjustType")){//矫正类别格式化
								colo=dictionaryInfoService.findItemsMap("JZLB").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("crimeType")){//犯罪类型格式化
								colo=dictionaryInfoService.findItemsMap("FZLX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("accusation")){//具体罪名格式化
								colo=dictionaryInfoService.findItemsMap("JTZM").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("oriPunishment")){//原判刑罚格式化
								colo=dictionaryInfoService.findItemsMap("XFLX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("imprisonmentPeriod")){//有期徒刑期限格式化
								colo=dictionaryInfoService.findItemsMap("YQTXQX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("addpunishment")){//附加刑格式化
								colo=dictionaryInfoService.findItemsMap("FJX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("nonpoliticalPeriod")){//剥夺政治权利期限格式化
							    colo=dictionaryInfoService.findItemsMap("BDZZQLQX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("sisType")){//是否“四史”格式化
								colo=dictionaryInfoService.findItemsMap("SS").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("sansType")){//是否“三涉”格式化
								colo=dictionaryInfoService.findItemsMap("SANS").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("receiveType")){//接收方式格式化
								colo=dictionaryInfoService.findItemsMap("JSFS").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("reportInfo")){//报到情况格式化
								colo=dictionaryInfoService.findItemsMap("BDQK").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("groupInfo")){//矫正小组人员组成情况格式化
								colo=dictionaryInfoService.findItemsMap("JZXZRYQK").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("removeReason")){//解除/终止矫正原因格式化
								colo=dictionaryInfoService.findItemsMap("JJYY").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("jailType")){//收监执行类型格式化
								colo=dictionaryInfoService.findItemsMap("SJZXLX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("jailReason")){//收监执行原因格式化
								colo=dictionaryInfoService.findItemsMap("SJZXYY").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("deathReason")){//死亡原因格式化
								colo=dictionaryInfoService.findItemsMap("SWYY").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("deathReasonDetail")){//具体死因格式化
								colo=dictionaryInfoService.findItemsMap("JTSY").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("performance")){//矫正表现格式化
								colo=dictionaryInfoService.findItemsMap("JZBX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("manner")){//认罪态度格式化
								colo=dictionaryInfoService.findItemsMap("RZTD").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("risk")){//危险性评估格式化
								colo=dictionaryInfoService.findItemsMap("WXXPG").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("familyContact")){//家庭联系情况格式化
								colo=dictionaryInfoService.findItemsMap("JTLXQK").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
							if(head[t].equals("alarmType")){//报警类型格式化
								colo=dictionaryInfoService.findItemsMap("BJLX").get(ViewCcFxryAllinfo[t]);
								ViewCcFxryAllinfo[t]=(colo!=null?colo:"");
								continue;
							}
						}
					}
					list.add((ViewCcFxryAllinfo));
				}
