...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		p.initsortorder = p.sortorder;
		p.initsortname = p.sortname;
		var ts= this, grid={
			headers:[],
			cols:[],
			footers: [],
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				var flag=-1;
				var nameflag = false;//sortname是否相同，相同时不进行sortname、sortorder重组
				if(pN.sort !== null) {
					prm[pN.sort]= ts.p.sortname;
					nameflag =  ts.p.initsortname!=ts.p.sortname;
					if(ts.p.initsortname!=null && ts.p.initsortname!='' && nameflag){
						var sortname = ts.p.sortname;
						var nameA = ts.p.initsortname.split(',');
						var result='';
						...
						if(flag<0){
							prm[pN.sort]= ts.p.sortname+','+ts.p.initsortname;
						}else{
							if(result!=''){
								prm[pN.sort]= ts.p.sortname+','+result.substring(0, result.length-1);
							}
						}
					}
				} 
				if(pN.order !== null) {
					...
					if(ts.p.initsortorder!=null && ts.p.initsortorder!='' && nameflag){
						if(flag<0){
							prm[pN.order]= ts.p.sortorder+','+ts.p.initsortorder;
						}else{
							var result='';
							var sortA = ts.p.initsortorder.split(',');
							...
							if(result!=''){
								prm[pN.order]=ts.p.sortorder+','+result.substring(0, result.length-1);
							}
						}
					}
				}
