...
<%@ include file="/common/commonjs.jsp"%>
	...
	function jqGridMergerCell(gridName,CellName){
		...
		$(function() {
			$.menu.init("#menuDetail",{first:"sqjz",second:"bbgl",third : "fxryReport"});
				//表格初始化
				$("#mainGrid").jqGrid({
					url		    : '${ctx}/data/jzgl/bbgl/statfxry/list.action',
					colNames	: [
					    '','年份', '季度','月份','状态','退回原因','操作'
					],
					...
					colModel	: [
					        {
						        ...
						        formatter:function(value, opts, rwdat){
						        	...
						        	var str = "<a onclick='javascript:viewQuarter(\""+rwdat.orgid+"\",\""+rwdat.year+"\",\""+quarter+"\")' title='' class='a-style'>";
						        		str += quarter;
						        		str+="</a>";
						        	return str;
						        },
			...
			function view(id,status){
				window.location = '${ctx}/data/jzgl/bbgl/list_detail.jsp?id='+id+'&status='+status;
			}
			...
			function openCreate(){
				$.container.popup({				
					items:[{
						save: "${ctx}/data/jzgl/bbgl/statfxry/add.action",
						title: '报表创建',
						fieldCols: 1,
						fieldCls:{labelCls : 'form-td-label-90',},
						fields:[
			...
			function viewCollect(id,orgId,year,month,orgType,status){
				...
				$.dialog({ 
					...
					height: '700px', 
					title:'社区服刑人员情况统计表查看',
				    content: 'url:<%=reportUrl%>data/jzgl/bbgl/tb_Report.jsp?raq='+raq+'&id='+id+'&orgId='+orgId+'&year='+year+'&month='+month+'&status='+status,
					cancelVal: '关闭', 
					cancel: function(){
						$("#mainGrid").trigger("reloadGrid");
					} 
			...
			function viewQuarter(orgId,year,quarter){
				var raq = 'fxryqk_jd';
				var status = '3';
				if(${user.wunit.orgType=='1'}){
					status='2';
				...
				$.dialog({ 
				    ...
				    width: '1800px', 
					height: '700px', 
					title:'社区服刑人员情况季度统计表查看',
				    content: 'url:<%=reportUrl%>data/jzgl/bbgl/jzgztj/tb_Report.jsp?view=1&raq='+raq+'&orgId='+orgId+'&year='+year+'&quarter='+quarter+'&status='+status,
					cancelVal: '关闭', 
					cancel: true
				});
			}
			function openDelete(id){
				$.dialog.confirm("确认要删除该条报表吗？", function() {
					$.ajax({
						type : "POST",
						async : false,
						url : "${ctx}/data/jzgl/bbgl/statfxry/delete.action",
						data : "id=" + id,
			...
			function openReport(id){
				$.dialog.confirm("确认要上报该条数据吗？", function() {
					$.ajax({
						type : "POST",
						async : false,
						url : "${ctx}/data/jzgl/bbgl/statfxry/reportTable.action",
						data : "id=" + id,
						success:function(data){
							$("#mainGrid").trigger("reloadGrid");
						}
...
<div class="main">
          <div class="breadcrumb-nav"> <span>您当前的位置：社区矫正 -> 报表管理->社区服刑人员情况统计表</span> </div>
