	...
	function jqGridMergerCell(gridName,CellName){
		...
		$(function() {
			$.menu.init("#menuDetail",{first:"sqjz",second:"bbgl",third : "ygztzjReport"});
				//表格初始化
				$("#mainGrid").jqGrid({
					url		    : '${ctx}/data/jzgl/bbgl/ygztzj/list.action',
					colNames	: [
					    '','年份', '月份','状态','退回原因','操作'
					],
					...
					colModel	: [
					        {
						        ...
						        formatter : function(value, opts, rwdat) {
						        	var str = '';
									if(${user.wunit.orgType=='1'}&&rwdat.status!='3'){
			...
			function view(id,status){
				window.location = '${ctx}/data/jzgl/bbgl/ygztzj/list_detail.jsp?id='+id+'&status='+status;
			}
			...
			function openCreate(){
				$.container.popup({				
					items:[{
						save: "${ctx}/data/jzgl/bbgl/ygztzj/add.action",
						title: '报表创建',
						fieldCols: 1,
						fieldCls:{labelCls : 'form-td-label-90',},
						fields:[
			...
			function openDelete(id){
				$.dialog.confirm("确认要删除该条报表吗？", function() {
					$.ajax({
						type : "POST",
						async : false,
						url : "${ctx}/data/jzgl/bbgl/ygztzj/delete.action",
						data : "id=" + id,
			...
			function openReport(id){
				$.dialog.confirm("确认要上报该条数据吗？", function() {
					$.ajax({
						type : "POST",
						async : false,
						url : "${ctx}/data/jzgl/bbgl/ygztzj/reportTable.action",
						data : "id=" + id,
						success:function(data){
							$("#mainGrid").trigger("reloadGrid");
						}
