...
<title>阳光中途之家当月工作统计表</title>
<%@ include file="/common/commonjs.jsp"%>
	<script type="text/javascript">
		$(function() {
			$.menu.init("#menuDetail",{first:"sqjz",second:"bbgl",third : "ygztzjReport"});
				//表格初始化
				$("#mainGrid").jqGrid({
					url		    : '${ctx}/data/jzgl/bbgl/ygztzj/list.action?opt=view&id=<%=request.getParameter("id")%>',
					colNames	: [
					    '','年份', '月份','机构','状态','操作'
					],
		...
		function view(id,orgId){
			$.dialog({ 
			    ...
			    width: '1800px', 
				height: '700px', 
				title:'阳光中途之家当月工作统计表查看',
			    content: 'url:<%=reportUrl%>data/jzgl/bbgl/ygztzj/tb_Report.jsp?status=3&raq=qxsfj_ygztzj&id='+id+'&orgId='+orgId,
				cancelVal: '关闭', 
				cancel: true 
			});
		}
		function sendBack(id){
			$.container.popup({				
				items:[{
					save: "${ctx}/data/jzgl/bbgl/ygztzj/sendBack.action",
					title: '退回原因填写',
					fieldCols: 1,
		...
		function sendSuccess(id){
			$.ajax({
				   type: "POST",
				   async:true,
				   url: "${ctx}/data/jzgl/bbgl/ygztzj/sendSuccess.action",
				   data:"id="+id,
		...
		function viewCollect(id,orgId,year,month,orgType,status){
			...
			$.dialog({ 
				...
				height: '700px', 
				title:'阳光中途之家当月工作统计表查看',
			    content: 'url:<%=reportUrl%>data/jzgl/bbgl/ygztzj/tb_Report.jsp?raq='+raq+'&id='+id+'&orgId='+orgId+'&year='+year+'&month='+month+'&status='+status,
				cancelVal: '关闭', 
				cancel: true 
			});
		}
...
<div class="main">
          <div class="breadcrumb-nav"> <span>您当前的位置：社区矫正 -> 报表管理->阳光中途之家工作统计表</span> </div>
