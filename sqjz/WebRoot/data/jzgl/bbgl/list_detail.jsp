	...
	<script type="text/javascript">
		$(function() {
			$.menu.init("#menuDetail",{first:"sqjz",second:"bbgl",third : "fxryReport"});
				//表格初始化
				$("#mainGrid").jqGrid({
					url		    : '${ctx}/data/jzgl/bbgl/statfxry/list.action?opt=view&id=<%=request.getParameter("id")%>',
					colNames	: [
					    '','年份', '月份','机构','状态','操作'
					],
		...
		function view(id,orgId){
			$.dialog({ 
				...
				height: '700px', 
				title:'社区服刑人员情况统计表查看',
			    content: 'url:<%=reportUrl%>data/jzgl/bbgl/tb_Report.jsp?status=3&raq=sfs_tj_tb_fxry&id='+id+'&orgId='+orgId,
				cancelVal: '关闭', 
				cancel: true 
			});
		}
		function sendBack(id){
			$.container.popup({				
				items:[{
					save: "${ctx}/data/jzgl/bbgl/statfxry/sendBack.action",
					title: '退回原因填写',
					fieldCols: 1,
		...
		function sendSuccess(id){
			$.ajax({
				   type: "POST",
				   async:true,
				   url: "${ctx}/data/jzgl/bbgl/statfxry/sendSuccess.action",
				   data:"id="+id,
