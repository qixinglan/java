//本脚本与organization.js基本一致
/*唯一修改if (!options.defValue && options.allowBlank === false && data.length > 0) {
				options.defValue =data[0].id;
				options.emptyText= data[0].codeDesc;
			}
 * 原来为if (!options.defValue && options.allowBlank === false && data.length > 0) {
...
(function($) {
	...
	$.fn.fmatter.org = function(cellval, opts) {
		var fn = $.org.formatter(cellval);
		return fn(cellval);
	};
})(jQuery);