...
$.extend($.roleDictionary,{
	load: function(url){
		var me = this;
		me.dict = {};
		$.ajax({
            type: "GET",
            url: url || "${ctx}/sys/user/roleJson.action",
            async: false,
            dataType: "json",
	...
	combobox: function(containerSelector, fieldname, code, options){
		...
		var fieldSelector = $('<input type="text" name="' + fieldname + '"/>');
...
//$.roleDictionary.load(CONTEXT_PATH + "/data/roleDictionary/json.action");

$.fn.fmatter.roleDictionary = function(cellval, opts) {
	if (opts.colModel && opts.colModel.formatoptions && opts.colModel.formatoptions.code){
		var fn = $.roleDictionary.formatter(opts.colModel.formatoptions.code, opts.colModel.formatoptions.emptyText);
		return fn(cellval);
	}
	if (opts.code){
		var fn = $.roleDictionary.formatter(opts.code, opts.emptyText);
		return fn(cellval);
	}
	return cellval;
};
